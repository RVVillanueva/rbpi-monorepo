CREATE OR REPLACE ALGORITHM = undefined SQL SECURITY definer VIEW `journal_audit_view` AS (select `acctng_journals`.`journalid` as `journal_id`, `acctng_journals`.`ibjournalid` as `ib_journal_id`, `acctng_journals`.`journal_date` as `journal_date`, `acctng_journals`.`journal_description` as `journal_description`, `acctng_journals`.`timestamp` as `posting_time`, `acctng_journals`.`journal_branch` as `branch_id`, `general_branches`.`name` as `branch_name`, `acctng_glaccounts`.`userlevel` as `user_level`, `acctng_journals`.`user` as `maker_id`, 
          CONCAT(
